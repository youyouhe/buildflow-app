(function() {
  ...
  if (document.getElementById('buildflow-badge-wrapper')) {
    return;
  }

  // Don't show badge on buildflow.vercel.app
  const hostname = window.location.hostname;
  const pathname = window.location.pathname;
  if (hostname === 'huggingface.co' && pathname.startsWith('/buildflow')) {
    return;
  }
  ...
  style.textContent = `
    ...
    #buildflow-badge-wrapper i::before {
      content: "";
      position: absolute;
      left: 50%;
      top: 50%;
    ...
    #buildflow-badge-wrapper:hover i::before {
      opacity: 1;
  ...
  link.href = 'https://buildflow.vercel.app';
  link.target = '_blank';
  link.rel = 'noopener noreferrer';
  
  // Create icon placeholder
  const icon = document.createElement('img');
  icon.src = 'https://buildflow.vercel.app/logo.svg';
  icon.alt = 'buildflow';
  ...
  // Create text
  const text = document.createTextNode('Made with buildflow');
